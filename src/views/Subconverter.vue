                  ...
                  <el-collapse-item>
                    <template slot="title">
                      <el-form-item label="高级功能:" style="width: 100%;">
                        <el-button type="primary" style="width: 100%;" icon="el-icon-more-outline">点击显示/隐藏
                        </el-button>
                      </el-form-item>
                    </template>
...
export default {
  ...
  created() {
    ...
    this.isPC = this.$getOS ? this.$getOS().isPc : true;
  },
  mounted() {
  ...
  methods: {
    ...
    makeUrl() {
      ...
      this.customSubUrl = backend + "/sub?target=" + this.form.clientType + "&url=" + encodeURIComponent(sourceSub) + "&insert=" + this.form.insert;
      ...
      this.customSubUrl += "&emoji=" + this.form.emoji.toString() + "&list=" + this.form.nodeList.toString() + "&xudp=" + this.form.xudp.toString() + "&udp=" + this.form.udp.toString() + "&tfo=" + this.form.tfo.toString() + "&expand=" + this.form.expand.toString() + "&scv=" + this.form.scv.toString() + "&fdn=" + this.form.fdn.toString();
    ...
    makeShortUrl() {
      ...
      const yourApiToken = process.env.VUE_APP_SHORT_API_TOKEN || "";
      ...
      if (duan.includes(yourApiDomain)) {
        ...
        this.$axios.post(duan, requestData, {
          headers: {
        ...
        this.$axios.post(duan, data).then(res => {
    ...
    confirmUploadConfig() {
      ...
      this.$axios.post(configUploadBackend, data).then(res => {
        if (res.data.code === 0 && res.data.data !== "") {
          ...
          this.$message.error("远程配置上传失败: " + (res.data.msg || "未知错误"));
        }
      }).catch(() => {
